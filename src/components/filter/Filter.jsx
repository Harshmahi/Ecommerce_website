...
function Filter() {
    ...
    // Function to reset filters to their placeholder values
    const resetFilters = () => {
        setFilterType('');
        setFilterPrice('');
    };

    // Set placeholder values for filter dropdowns when no filters are applied
    const placeholderType = product.length ? 'All Categories' : 'Loading...';
    const placeholderPrice = product.length ? 'All Prices' : 'Loading...';
            ...
            <div className='container mx-auto px-4 mt-5'>
                <div className="p-5 rounded-lg bg-gray-100 drop-shadow-xl border border-gray-200" style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '' }}>
                    ...
                    <div className="flex items-center justify-between mt-4">
                        ...
                        <button onClick={resetFilters} className="px-4 py-2 bg-gray-50 hover:bg-gray-200 text-white-800 text-sm font-medium rounded-md" style={{ color: mode === 'dark' ? 'gray' : '' }}>
                            Reset Filter
                        </button>
                    </div>
                    <div>
                        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 mt-4">
                            ...
                            <select
                                ...
                                style={{ backgroundColor: mode === 'dark' ? 'rgb(64 66 70)' : '', color: mode === 'dark' ? 'white' : '' }}
                            >
                                <option value="">{placeholderType}</option>
                            ...
                            <select
                               ...
                               style={{ backgroundColor: mode === 'dark' ? 'rgb(64 66 70)' : '', color: mode === 'dark' ? 'white' : '' }}
                           >
                               <option value="">{placeholderPrice}</option>
                               {product.map((item, index) => (
